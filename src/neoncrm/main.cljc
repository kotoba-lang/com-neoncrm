(ns neoncrm.main
  "Kotodama WASM entrypoint for the Neoncrm clean-room actor (L4) — Clojure port.
  ...
  facts (`neoncrm.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "neoncrm")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"neoncrm.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "neoncrm-compat" :tier tier :entities entities} 200])
...
(defn start! [] :neoncrm-compat/ready)
